...
class CommandHandler:
    ...
    def handle_command(self, command, args):
        if command == 'disable_net':
            self.disable_network(*args)
        elif command == 'enable_net':
            self.enable_network(*args)
        else:
            raise Exception("Unknown command")

    def disable_network(self, interface):
        self.controller.disable_network(interface)
    
    def enable_network(self, interface):
        self.controller.enable_network(interface)
